...
function SearchWithAi() {
  ...
  <div className="bg-white shadow-xl rounded-3xl p-6 sm:p-8 w-full max-w-2xl text-center relative">
    <FaArrowLeftLong
      onClick={() => navigate("/")}
      className="text-teal-600 hover:text-teal-700 w-[22px] h-[22px] cursor-pointer absolute left-4 top-4 transition-all"
    />
    <h1 className="text-2xl sm:text-3xl font-bold text-gray-800 mb-6 flex items-center justify-center gap-2">
      ...
      />
      Search with <span className="text-teal-600">AI</span>
    </h1>
    ...
    <div className="flex items-center bg-gray-700 rounded-full overflow-hidden shadow-lg relative w-full">
      ...
      {input && (
        <button
          ...
          className="absolute right-14 sm:right-16 bg-teal-600 hover:bg-teal-700 rounded-full transition-all"
        >
          <img
            src="/assets/ai.png"
      ...
      <button
        ...
        className="absolute right-2 bg-teal-600 hover:bg-teal-700 rounded-full w-10 h-10 flex items-center justify-center transition-all"
      >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 sm:gap-8">
        {recommendation?.map((course, index) => (
          <div
            ...
            key={index}
            className="bg-white text-black p-5 rounded-2xl shadow-md hover:shadow-teal-500/30 transition-all duration-200 border border-gray-200 cursor-pointer hover:bg-teal-50"
          >
